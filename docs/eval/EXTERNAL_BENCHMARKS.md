...
Missing capabilities across the 38 class-3 templates (multi-tagged). Two
columns, because six capability sessions have shipped since D-050 and a
July count presented as current is the error this project keeps catching in
its own claims. **No template has changed class** in any of them — the
right-hand column is a re-audit of the *tags*, regenerated by
`python3 scripts/ldbc_fit.py report`:

| tag | D-050 (July) | now (L18) | what it is |
|---|---:|---:|---|
| `PAT` | 35 | 35 | labelled multi-way pattern matching |
| `PROP` | 33 | 26 | property predicates and arbitrary attribute projection |
| `AR` | 16 | — | arithmetic beyond count/sum/min/max/topk — *shipped* (D-051, D-055), and it was three capabilities wearing one label |
| `G` | 15 | 16 | grouping past two dimensions, or regrouping a result |
| `SET` | 13 | 5 | set operations and joins between result sets — the join half shipped in D-055 |
| `PROJ` | — | 8 | projecting a property *value* into output rows, split out of `PROP` by D-052 |
| `NEG` | 6 | 6 | absence conditions |
| `SP` | 6 | 6 | shortest / all-shortest / cheapest paths |
| `CAL` | 5 | 5 | calendar semantics — and after D-057 shipped the *cyclic* units (hour-of-day, weekday, month-of-year) all five of these turn out to want **absolute** ones: a `workFrom` year, a calendar-year grouping, a per-calendar-month rate, two given dates, a birthday month-and-day |
| `ROW` | — | 1 | one residual: `derive` has no `abs`, which BI2 orders by |

`PAT` and `PROP` together gate **37 of the 38**, in July and now. Two structural facts do most
