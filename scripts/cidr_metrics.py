...
one gated-type claim (count, value, entity, ordering) — the same definition
the arXiv paper uses (coverage 199/282 on CollegeMsg ours reproduces).
...
def main(dirs: list[str]) -> None:
    for d in dirs:
        ...
        for sys_name in sorted(rows_by):
            ...
            ucr_known = [r for r in emit if r.get("ucr") is not None]
