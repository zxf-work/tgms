...
**Correction-aware result freshness.** `tgms trace check` tells you whether a correction landed after an answer was computed could have changed it — "this result was computed then; a correction received since affects data it depended on; reconsider it" — and it never calls a stale answer fresh: 0 false-fresh verdicts on 447 changed answers in the M4 injection campaign (run 2, the record of account) and 0 more on 1,852 changed answers in the M5 carve-2 campaign (28,044 trials), where the naive row-touch check is wrong 47% of the time on the M4 population.
