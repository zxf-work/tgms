...
Usage:
    ...
    uv run python scripts/eval_fault_matrix.py --dry-run
    uv run python scripts/eval_fault_matrix.py --suite collegemsg \\
...
def gated_run(answer_obj: dict[str, Any], trace: Any, verifier: ClaimVerifier,
    ...
    gate out `unsupported` claims exactly as the deployed 'ours' system
    does (`gate_answer`), and classify what actually reaches the user. A
    claim gated to nothing (the only claim, dropped) is a failure to
    answer, not a vacuously-correct empty answer -- represented as
    ...
    gated, gated_report, n_dropped = gate_answer(answer_obj, aug)
    if not gated["claims"] and n_dropped:
        err = {"error": "E_GATED_EMPTY",
              "message": "every claim was gated out as unsupported before "
                          "delivery", "details": {"n_dropped": n_dropped}}
        return classify(Run(error=err), oracle)
    run = Run(answer=gated, answer_value=trace.answer, report=gated_report)
    return classify(run, oracle)
...
def run_agent_plan_trial(plan_json: dict[str, Any], task_input_uids: set[str],
    ...
    cls = gated_run(ans, trace, verifier, Oracle(gold=gold))
    return {"stage": "answer", "mutant": mutant, **cls.to_json()}
...
def run_answer_level_cell(cell_id: str, tasks: list[dict[str, Any]], store: Any,
                          scratch: Path, n: int, seed: int) -> list[dict[str, Any]]:
    if cell_id == "F1-11":
        ...
        for c in cases:
            ...
            out.append({"stage": "answer", "task_id": c["task_id"], **cls.to_json()})
        return out
    ...
    while len(trials) < n and attempts < max_attempts:
        ...
        cls = gated_run(mutant, trace, verifier, oracle)
        trials.append({"stage": "answer", "mutator": name, **cls.to_json()})
    return trials
...
def run_exec_fault_cell(cell_id: str, tasks: list[dict[str, Any]], store: Any,
    ...
    while len(trials) < n and attempts < max_attempts:
        ...
        verifier = ClaimVerifier(trace, results, store.adapter)
        cls = gated_run(ans, trace, verifier, Oracle(gold=task.get("gold")))
...
def run_cell(cell_id: str, suite: dict[str, Any], store: Any, scratch: Path,
            ...
            surface: str | None = None) -> list[dict[str, Any]]:
    ...
    if cell_id in PLAN_MUTATORS:
        return run_plan_cell(cell_id, PLAN_MUTATORS[cell_id], tasks, store, scratch, n, seed)
    if cell_id in ANSWER_LEVEL_MUTATOR_NAMES or cell_id == "F1-11":
        return run_answer_level_cell(cell_id, tasks, store, scratch, n, seed)
    if cell_id in EXEC_FAULTS or cell_id == "F2-5":
        return run_exec_fault_cell(cell_id, tasks, store, scratch, n, seed)
...
def build_manifest(cell_id: str, suite_name: str, suite_path: Path, seed: int,
                   n: int, trials: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "protocol": "model-free (primary arm, section 5): oracle plans "
                   "mutated post-planner, no LLM",
