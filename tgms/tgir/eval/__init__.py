...
from tgms.tgir.node import (
    EdgeScan, Filter, Join, Limit, Node, NodeScan, Order, Project,
...
class Execution:
    ...
    def __init__(self, adapter: Any, live: LiveMap | None = None) -> None:
...
def evaluate_core(node: Node, adapter: Any) -> Relation:
    """Evaluate a compositional plan rooted at `node`.

    Column pruning runs once, before execution, over the whole reachable plan.
    """
    return Execution(adapter, live_columns(node)).run(node)


__all__ = [
    "Execution", "PENDING", "eval_expr", "eval_filter", "eval_join", "eval_limit",
    "eval_order", "eval_predicate", "eval_project", "eval_property_predicate",
