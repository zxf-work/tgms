...
class EvolutionMemory:
    ...
    def build(self, adapter: StorageAdapter, stride: int = DEFAULT_STRIDE,
              ...
              model: str = "") -> int:
        """Summarize every stride window over the dataset extent; returns the
        number of notes stored. Idempotent: existing windows are replaced."""
        stats = adapter.stats()
        ...
        while t < vt_max:
            ...
            if llm_fn is not None:
                prompt = [
                    {"role": "system", "content":
                     ...
                     "round any number. Output the summary text only."},
            ...
            self.conn.execute(
                ...
                "payload_json, digest_text) VALUES (?, ?, 'window_digest', ?, ?)",
                (t, t_end, canonical_json(facts), text))
            n += 1
            t = t_end
        self.conn.commit()
        return n
    ...
    def retrieve(self, t_a: int, t_b: int, k: int = 3,
        ...
        """Deterministic retrieval: notes overlapping [t_a, t_b), ranked by
        overlap ratio (ties: earlier window first)."""
        rows = self.conn.execute(
            "SELECT window_ta, window_tb, kind, payload_json, digest_text "
            "FROM memory_notes WHERE kind = ? AND window_ta < ? AND window_tb > ? "
            "ORDER BY window_ta", (kind, t_b, t_a)).fetchall()
        scored = []
        for ta, tb, kd, payload, text in rows:
